...
- A completed visit is not recorded the moment its last dart lands: it waits out `visitPause` (two seconds, `debugVisitPause` in tests) as `_pendingVisit`, still on the board, with `inputLocked` true. Nothing about the game changes until `_settleVisit` runs, which is why undo in that window is just a dart off the board and why `leaveGame` and the app going to the background flush it first: a pending visit exists nowhere else. The bot waits out the same pause and never schedules while one is pending
