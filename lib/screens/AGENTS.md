...
- The input of every mode reads `inputLocked` and goes dim and deaf while it holds; the provider refuses the dart either way, the widget only says so. While `visitPending` holds, a `VisitPauseBar` under the darts shows the wait running out and the throw buttons make way for a Continue button that calls `flushHeldVisit`, in X01 through its own `_ActionButton`, elsewhere through `ContinueButton`
