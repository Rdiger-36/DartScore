...
- A completed visit stays on the board for `TurnPacing.visitPause` before the turn moves on, in every mode, for people and bots alike, and a tap on the dart row moves on at once. The pause and the bot's delay follow the `GamePace` the settings keep in `PaceProvider`, read by the game providers through `TurnPacing.pace`. In X01 it is not even recorded until then, so leaving the game and the app going to the background flush it (`leaveGame`, `flushHeldVisit`); the other modes record each dart as it lands and only hold the turn. Tests zero the pause through `TurnPacing.debugVisitPause`, which `useInMemoryDatabase` does for them
