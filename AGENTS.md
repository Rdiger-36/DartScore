...
- A completed visit stays on the board for `TurnPacing.visitPause` before the turn moves on, in every mode, for people and bots alike. In X01 it is not even recorded until then, so leaving the game and the app going to the background flush it (`leaveGame`, `flushHeldVisit`); the other modes record each dart as it lands and only hold the turn. Tests zero the pause through `TurnPacing.debugVisitPause`, which `useInMemoryDatabase` does for them
